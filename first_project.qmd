---
title: "Credit Card Default: Probabilistic Tool Analysis"
format:
    html:
        code-fold: true
        toc: true
...
* Correlations weaken with temporal distance : `bill_sep` and `bill_apr` are still moderately correlated (0.80), but less so than neighbouring months.
...
## 3.5 Repayment Status by Default

The six `status_*` columns encode each client's repayment behaviour for a given month: −2 = no consumption, −1 = paid in full, 0 = revolving credit used, 1–8 = months of payment delay. Because late payment is a direct precursor to default, these are likely to be among the most discriminative features in the dataset. We compare their distributions across the two target classes using the most recent month (`status_sep`) and then show the full six-month picture.

```{python}
status_cols_ordered = ['status_apr', 'status_may', 'status_jun',
                       'status_jul', 'status_aug', 'status_sep']

fig, axes = plt.subplots(1, 2, figsize=(14, 5))

# Distribution of status_sep by default class
for label, grp in df.groupby('default'):
    axes[0].hist(grp['status_sep'], bins=range(-2, 10),
                 alpha=0.6, density=True,
                 label=f"{'Default' if label == 1 else 'No Default'}")
axes[0].set_title('September Repayment Status by Default Class')
axes[0].set_xlabel('status_sep (−2 = no use, −1 = paid full, 1–8 = months late)')
axes[0].set_ylabel('Density')
axes[0].legend()
axes[0].grid(True, alpha=0.3)

# Mean repayment status per month, split by default class
mean_status = (df.groupby('default')[status_cols_ordered]
                 .mean()
                 .T
                 .rename(columns={0: 'No Default', 1: 'Default'}))
mean_status.index = ['Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep']

axes[1].plot(mean_status.index, mean_status['No Default'], marker='o', label='No Default')
axes[1].plot(mean_status.index, mean_status['Default'],    marker='s', label='Default')
axes[1].set_title('Mean Repayment Status Over Time by Default Class')
axes[1].set_ylabel('Mean status code')
axes[1].legend()
...
* **`status_sep` is strongly discriminative**: non-defaulters cluster predominantly at **0** (revolving credit used), while defaulters are shifted markedly toward positive values.
* **Temporal trend confirms escalation**: the mean repayment status for defaulters rises steadily from April (~0.1) to September (~0.65), while non-defaulters remain negative throughout, reflecting a mix of no-consumption and revolving-credit behaviour.
...
We explicitly categorise every feature before applying transformations, ensuring the right treatment for each variable type. We include age on `monetary_cols` as these are the ones that are going to be log transformed.
...
**Interpretation:** For all transformed columns, absolute skewness dropped substantially, bringing the distributions much closer to symmetry. This will improve the Gaussian assumptions underlying LDA and Naïve Bayes and reduce the influence of outliers on distance-based models.
...
for name, model in [('Original (no resampling)', lda_orig),
    ...
    resample_rows.append({
        ...
        f'Precision@{opt_t:.2f}':    precision_score(y_te_np, yp_opt, zero_division=0),
...
prec_col = f'Precision@{opt_t:.2f}'
prec_down = resample_df.loc['Random undersampling', prec_col]
prec_up   = resample_df.loc['Random oversampling',  prec_col]
prec_smote= resample_df.loc['SMOTE',                prec_col]
```
...
* **Balanced priors** are the best single model-level fix: one parameter change raises recall to **60 %** and F1 to **0.517** (the highest F1 of any model-level intervention, and second-highest overall behind Youden's J threshold at 0.519) with expected cost of 0.601, essentially tied with threshold tuning and more principled theoretically.
...
* **Resampling at threshold 0.5** brings comparable gains to balanced priors (recall around 61 %, expected cost around 0.61), but combining resampling with the cost-optimal threshold backfires: the model overshoots to around 98 % recall with precision collapsing to around **`{python} f"{prec_down:.2f}"`**, worsening expected cost to 0.73.
...
* The ROC-AUC of 0.7158 indicates meaningful ranking ability (better than random), but thresholding at 0.50 leaves many false negatives. This motivates tuning k via cross-validation (Section 8.3).
...
k_grid = list(range(1, 62, 2))   # odd ks up to 61 to confirm the plateau
...
for k in k_grid:
    ...
    for tr_idx, va_idx in cv.split(X_train_scaled, y_train):
        ...
        accs.append(accuracy_score(y_va, m.predict(X_va)))
...
knn_cv_df = pd.DataFrame(rows)
knn_cv_auc_df = knn_cv_df.sort_values("cv_auc_mean", ascending=False)
display(knn_cv_auc_df[["k", "cv_auc_mean", "cv_auc_sd"]].head(10))

best_k = int(knn_cv_auc_df.iloc[0]["k"])
...
fig, ax = plt.subplots(figsize=(8, 4))
ax.plot(tmp["k"], tmp["cv_acc_mean"], marker="o")
...
* As k increases, the mean CV ROC-AUC improves rapidly at first (by around k ≈ 5 it is already about 0.71), then increases more gradually and plateaus for larger k.
...
* The shaded band (mean ± 1 sd) indicates that variability across folds is relatively small once k is moderate, and the curves for large k overlap substantially. The extended grid (up to k=61) confirms that the curve has genuinely plateaued, so the selected k is not a boundary artefact; nearby values would perform very similarly.
* The curve flattens noticeably after roughly k ≈ 20, where improvements become marginal. This suggests that once the neighbourhood is large enough, adding more neighbours produces only small gains in ranking performance.
* The best mean CV ROC-AUC occurs at the selected k with a standard deviation of about 0.0102, confirming stability across folds. Nearby values are extremely close, indicating that performance is fairly stable in the high-k region.
* We take k=59 forward into the final test evaluation (Section 8.4).
...
* The cross-validated choice k=59 generalises well: the mean CV ROC-AUC was 0.7626 and the test ROC-AUC is 0.7575, a small drop of about 0.005, suggesting limited overfitting.
* Relative to the baseline k=5 (ROC-AUC 0.7158), the tuned model improves ranking performance by about 0.042 in ROC-AUC and slightly increases accuracy (0.81 vs 0.80). This confirms that tuning k materially improves discrimination.
* From the confusion matrix (TN = 6705, FP = 304, FN = 1442, TP = 549), the model is even more conservative at the default 0.50 threshold: it predicts “Default” only 853 / 9000 = 9.48% of the time, well below the true default rate (22.1%).
* For the Default class, precision is 0.64, but recall drops to 0.28 (approximately 72 % of defaulters are missed). In other words, increasing k smooths the decision rule and reduces false positives, but increases false negatives. **This conservatism is partly structural**: with 22 % defaulters in the training set, a neighbourhood of k=59 will on average contain roughly 13 defaulters and 46 non-defaulters. Majority-vote at threshold 0.5 therefore requires defaulters to constitute more than half the neighbours. Applying the cost-sensitive threshold (≈ 0.20) from Section 7 to the k-NN probability outputs would resolve this in the same way as it did for LDA, raising Default recall substantially without retraining.
* Performance on the majority class remains strong (No Default recall 0.96), which helps overall accuracy but again highlights that accuracy alone is not sufficient under class imbalance. 
* The combined ROC plot shows that the tuned model dominates the baseline across most thresholds: AUC increases from 0.716 (k=5) to 0.757 (k=59), confirming that tuning k improves the model’s ability to rank defaulters ahead of non-defaulters.
* The Precision–Recall plot provides a clearer view under class imbalance. Average precision increases substantially from 0.422 (k=5) to 0.515 (k=59), meaning the tuned model achieves better precision at a given recall level over a wide range of thresholds. This improvement in ranking contrasts with the low Default recall at the fixed 0.50 threshold, highlighting that the main limitation is the chosen decision threshold rather than the model’s probability ordering.
...
For probabilistic classifiers, the results show a clear trade-off between overall accuracy and minority-class detection. LDA achieved the best overall ranking among the generative models (ROC-AUC ≈ 0.7401, accuracy ≈ 0.8066) but was conservative, with Default recall (≈ 0.2697) at threshold 0.50. QDA improved Default recall (≈ 0.4902) at the expense of lower ROC-AUC (≈ 0.7151) and lower accuracy (≈ 0.7520), consistent with the added flexibility being limited by correlated billing features. Gaussian Naïve Bayes offered the best minority-class balance among these three (Default recall ≈ 0.5113, Default F1 ≈ 0.4891) while maintaining an ROC-AUC comparable to LDA (≈ 0.7396). 

Because the application is credit risk, Section 7 demonstrated that performance should be evaluated under asymmetric error costs. Using a cost matrix with false negatives more expensive than false positives yields a theoretical cost-optimal probability threshold of $p^* = 0.167$.  The highest-leverage intervention was threshold tuning on LDA probabilities: moving from the default threshold 0.50 (expected cost ≈ 0.8397) to a cost-aligned threshold around 0.20 reduced expected cost to about 0.5990 and raised Default recall to about 0.6339.  Balanced priors provided a similarly strong, model-level alternative with expected cost around 0.601, while resampling (undersampling/oversampling/SMOTE) improved recall at threshold 0.50 but could overshoot under the cost-optimal threshold, worsening expected cost. 

Finally, k-NN showed that non-parametric models can improve discrimination when tuned: cross-validation selected k=59, and test ROC-AUC increased to 0.7575 (vs 0.7158 for k=5). However, at the default 0.50 threshold the tuned k-NN remained conservative (Default recall ≈ 0.28), reinforcing the broader finding that ranking performance (ROC-AUC) and decision performance (cost-sensitive classification) are not the same objective.

**Overall recommendation:** for an operational decision rule under the stated 5:1 cost asymmetry, the strongest and simplest solution in this report is to use LDA probability outputs with a cost-sensitive threshold (≈ 0.20) (or equivalently balanced priors), because it delivers the lowest expected cost while substantially increasing Default recall.
