...
To keep grid-search runtime tractable, we tune on a stratified 8 000-row subsample of the training set and refit the best configuration on the full 21 000-row training set.
...
* Accuracy at the default threshold (81.0 %) and recall on the default class (28.5 %) are marginally better than the linear SVM's, but the drop in AUC means the model's overall ranking quality is worse.
...
* Encoding the 5:1 cost ratio directly via `class_weight={0:1, 1:5}` is the most effective SVM strategy. It raises recall on defaulters to **71.7 %** — almost triple the standard RBF SVM's 28.5 % — and achieves the **lowest expected cost among all SVM variants (0.5736)**.
...
* The pruning path contains **1 312 candidate α values**; cross-validation selects **α = 0.000177**, producing a tree with **14 levels** and **83 leaves**. This is substantially pruned from the full tree but still large enough to capture complex interactions.
...
print("\n=== Bagging (300 trees): Classification Report ===")
...
* The improvement over the single tree is consistent but modest: AUC drops slightly (0.7544 vs 0.7568 for the pruned tree) because all bagging trees use the full feature set at each split, leading to high correlation among trees. The dominant features (repayment status variables) appear at the top of nearly every tree, limiting diversity.
...
* At the cost-optimal threshold 0.20, recall reaches **66.1 %** with the **lowest expected cost of any tree-based method (0.5604)** — a 6.5 % improvement over Part 1's LDA benchmark (0.599).
...
* The hyperparameter search ranks six configurations by validation AUC after 10 quick epochs. The winning configuration is **hidden1 = 256, hidden2 = 128, dropout = 0.2, lr = 1e-3** (val AUC = 0.7871), but the top four configs are within 0.004 of each other, suggesting that performance is not highly sensitive to these architecture choices on this dataset.
* Higher learning rates (1e-3 vs 5e-4) consistently outperform lower ones across all hidden sizes, likely because the quick-search epochs (10) are too few for the slower learning rate to converge. Dropout of 0.1 performs similarly to 0.2, while 0.3 with a slow learning rate underperforms.
* The final tuned MLP (256 → 128, 40 epochs) achieves **ROC-AUC = 0.7649** — slightly below the baseline's 0.7716. This apparent paradox is due to **overfitting**: the learning curves show the training AUC reaching **0.8437** by epoch 40 while validation AUC peaks around epoch 15–20 and then declines to 0.7705. The larger network has more capacity to memorise training data.
* At the cost-optimal threshold 0.20, expected cost is **0.5822**, placing the MLP between GBM/XGBoost (0.560) and Random Forest (0.588).
* The MLP's performance on this moderately-sized tabular dataset (21 000 rows, 24 features) is respectable but does not surpass gradient boosting. This is consistent with the broader machine learning literature, where tree-based methods tend to dominate on structured/tabular data while neural networks excel on unstructured data (images, text, time series).
...
if isinstance(shap_values, list):
    shap_arr = shap_values[0]   # shape (n_explain, n_features)
else:
...
shap_arr  = np.squeeze(shap_arr)   # ensure (n_samples, n_features); drops trailing dim from wrapper
...
* The **SHAP beeswarm plot** reveals that the MLP's top features are the same repayment-status variables (`status_sep`, `status_aug`, etc.) that dominated the Random Forest's Gini importance. High positive SHAP values (pushing the logit toward "default") are associated with high feature values (i.e. more months of delayed payment), confirming that the MLP has learned an economically intuitive relationship.
...
* The **side-by-side comparison** between MLP SHAP ranking and RF Gini importance shows strong agreement in the top features but some divergence in the middle ranks. For instance, the MLP may weight certain bill-amount features differently from the RF, reflecting the non-linear combinations the network can learn. Despite this, the overall feature hierarchy is remarkably consistent across model families, reinforcing the conclusion that **payment history is the dominant predictor of default** in this dataset.
...
* The full comparison table reveals a clear **ranking by ROC-AUC**: GBM and XGBoost lead at **0.7809**, followed by MLP (0.7649), Random Forest (0.7599), cost-sensitive SVM (0.7576), k-NN (0.7575), Decision Tree (0.7568), Bagging (0.7544), LDA (0.7401), GNB (0.7396), Linear SVM (0.7394), and RBF SVM (0.7082).
* When evaluated by **expected cost** at the cost-optimal threshold (the metric most relevant to the bank's decision problem), the ranking shifts: **GBM (t = 0.20)** achieves the lowest expected cost at **0.5604**, followed by XGBoost (0.5622), cost-sensitive SVM (0.5736), MLP (0.5822), and Random Forest (0.5884). All of these improve substantially over Part 1's best result (LDA at t = 0.20, expected cost 0.599).
...
* A consistent pattern across all models: at the standard threshold (t = 0.50), all models have accuracy ≈ 80–82 % but very low recall on defaulters (24–39 %). The cost-optimal threshold (t = 0.20) consistently raises recall to 63–72 % at the cost of overall accuracy, reducing expected cost by 20–30 %. This demonstrates that **threshold calibration is the single most impactful intervention** — more important than model choice for any model that already produces well-calibrated probabilities.
* The cost-sensitive SVM (class_weight) is the exception: it achieves a competitive expected cost of **0.5736** even at the standard threshold, because the cost asymmetry is baked into training rather than applied at prediction time.
...
**2. The MLP neural network is competitive but does not surpass tree-based ensembles on this structured data.** The tuned MLP achieves ROC-AUC = 0.7649, placing it between Random Forest and GBM. This outcome aligns with the broader empirical finding that gradient boosting tends to outperform deep learning on moderate-sized tabular datasets. The MLP's tendency to overfit (training AUC 0.84 vs validation AUC 0.77 after 40 epochs) suggests that additional regularisation (e.g. early stopping, weight decay) or a larger dataset would be needed to close the gap.

**3. Threshold calibration is more impactful than model selection.** Moving from the default threshold (0.50) to the cost-optimal threshold (0.20) consistently reduces expected cost by 20–30 % across all probabilistic models. For a bank, this means that the choice of probability threshold is at least as important as the choice of algorithm.

**4. Cost-sensitive training (class weighting) is an effective alternative to threshold tuning.** The SVM with class_weight={0:1, 1:5} achieves expected cost 0.5736 at the standard threshold — comparable to GBM at its cost-optimal threshold — because the cost asymmetry is incorporated directly into the objective function during training.
